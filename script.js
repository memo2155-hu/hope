...
function updateRanking(){
  ...
  let data=names.map(n=>{
    let t=JSON.parse(localStorage.getItem("user_"+n+"_tasks")||"[]").length;
    let h=JSON.parse(localStorage.getItem("user_"+n+"_homeworks")||"[]").length;
    let no=JSON.parse(localStorage.getItem("user_"+n+"_notes")||"[]").length;

    return {name:n,score:t+h+no};
  });
